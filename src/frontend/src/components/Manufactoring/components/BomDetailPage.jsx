// packages/app/src/renderer/features/boms/components/BomDetailPage.jsx
...
export default function BomDetailPage() {
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try {
      const res = await api.deleteBom(bom.id);
      if (!res.success) {
        toast.error(res.error);
        return;
      }
      toast.success(t("screens.boms.deleted", "BOM deleted"));
      navigate("/boms");
    } finally {
      setDeleting(false);
      ...
      <div className="min-h-screen bg-[#f6f8fd] p-5">
        <div className="mx-auto max-w-4xl">
          <div className="flex items-start gap-2.5 rounded-xl border border-red-200 bg-red-50 px-3.5 py-2.5 text-xs font-bold text-red-700">
            ...
            <span>{error || t("screens.boms.notFound", "BOM not found")}</span>
          </div>
          <button
            type="button"
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs font-semibold text-slate-500">
                {bom.product_name}
                {bom.product_code ? ` · #${bom.product_code}` : ""}
              </p>
            </div>
          </div>
        ...
        {bom.notes && (
          <section className={panelClass}>
            ...
            <div className={`${panelBodyClass} flex items-start gap-2.5`}>
              ...
              <p className="text-sm font-medium text-slate-700">{bom.notes}</p>
            </div>
          </section>
        )}
                ...
                {bom.items.map((item) => (
                  <tr key={item.id} className="transition hover:bg-[#fafbff]">
                    ...
                    <td className="px-4 py-2.5 font-bold tabular-nums text-red-600">
                      {money(item.line_estimated_cost || 0)}
                    </td>
                  </tr>
                ))}
              </tbody>
