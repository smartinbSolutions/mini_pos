...
export default function PrintExpense() {
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all([
      window.api.getExpense(id),
      window.api.getCompanySetting(),
    ]).then(([expenseData, companyRes]) => {
      if (!cancelled) {
  ...
  const hasAnyTax =
    items.some((i) => Number(i.tax_rate) > 0) || Number(expense.taxValue) > 0;
  const hasAnyDiscount =
    items.some((i) => Number(i.discount) > 0) || Number(expense.discount) > 0;
  ...
  const hasDeductions = itemDiscountTotal > 0 || Number(expense.discount) > 0;
        ...
        <div className="flex justify-between">
          ...
          <div className="w-2/5 text-start">
            ...
            <p className="font-mono tabular-nums text-base font-semibold mb-2">
              {expense.invoice_name || `#${expense.id}`}
            </p>
            <div
      ...
      <div className="rounded border border-[#E5E5E2] p-4 mb-4">
        <div className="flex justify-between gap-6">
          <div className="w-1/2">
            ...
            <p className="text-[#6B6F76]">{expense.supplier_phone}</p>
          </div>
          ...
          <div className="w-1/2">
            ...
            <p className="font-medium">{expense.created_by_name}</p>
          </div>
        </div>
      </div>
        ...
        <table className="w-full border-collapse text-xs">
          <thead>
            <tr className="border-b border-[#33363D]">
              <th className="p-2 text-start font-medium">{t("ui.category")}</th>
          ...
          {hasAnyTax && (
            <div className="mt-2 rounded bg-[#FBF7EF] px-3 py-2 space-y-1.5">
              ...
              <div className="flex justify-between text-[11px] font-medium text-[#8A6A32] pt-1.5 border-t border-[#EADFC5]">
                ...
                <span className="font-mono tabular-nums">
                  +{money(itemTaxTotal + Number(expense.taxValue || 0))}
                </span>
              </div>
            </div>
          )}
