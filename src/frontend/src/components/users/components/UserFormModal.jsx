...
import { AlertCircle, KeyRound, Save, User, UserCog, X } from "lucide-react";
...
const UserFormModal = ({
  ...
  const inputClass =
    "h-11 w-full rounded-xl border border-[#dbe4ff] bg-white/90 px-3.5 text-sm text-slate-800 outline-none transition placeholder:text-slate-400 focus:border-[#4663ff] focus:ring-4 focus:ring-[#4663ff]/10";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div
        role="dialog"
        aria-modal="true"
        className="relative z-10 w-full max-w-md overflow-hidden rounded-2xl border border-[#e5ebff] bg-white shadow-2xl animate-in fade-in zoom-in-95"
      >
        ...
        <form onSubmit={handleSubmit}>
          <div className="space-y-4 px-5 py-5">
            ...
            <label>
              <span className={labelClass}>{t("screens.users.fullName")}</span>
              <input
                name="full_name"
            ...
            <label>
              <span className={labelClass}>{t("screens.users.role")}</span>
                  ...
                  <button
                    ...
                    className={`h-11 rounded-xl border text-sm font-bold transition ${
                      form.role === r
                        ...
                        : "border-[#dbe4ff] bg-white text-slate-600 hover:border-[#b9c6ff]"
                    }`}
                  >
          ...
          <div className="flex flex-col-reverse gap-3 border-t border-slate-100 bg-slate-50 px-5 py-4 sm:flex-row sm:justify-end">
            ...
            <button
              type="submit"
              disabled={saving}
              className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-[#4663ff] px-5 text-sm font-black text-white shadow-lg shadow-[#4663ff]/20 transition hover:bg-[#3854e8] disabled:cursor-not-allowed disabled:opacity-50"
            >
              <Save size={16} />
